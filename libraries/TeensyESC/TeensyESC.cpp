...
/*
  TeensyESC.cpp - Hardware Servo Timer Library
  http://arduiniana.org/libraries/TeensyESC/
  Author: Jim Studt, jim@federated.com
  Copyright (c) 2007 David A. Mellis.  All right reserved.
  renamed to TeensyESC by Mikal Hart
...
uint8_t TeensyESC::attachedA = 0;
uint8_t TeensyESC::attachedB = 0;
#ifdef SERVO_PIN_C
uint8_t TeensyESC::attachedC = 0;
#endif

void TeensyESC::seizeTimer1()
{
...
void TeensyESC::releaseTimer1() {}

TeensyESC::TeensyESC() : pin(0), angle(NO_ANGLE) {}

uint8_t TeensyESC::attach(int pinArg, int min, int max)
{
  #ifdef SERVO_PIN_C
...
void TeensyESC::write(int angleArg)
{
  uint16_t p;
...
uint32_t TeensyESC::attachedpins[(NUM_DIGITAL_PINS+31)/32]; // 1 bit per digital pin

TeensyESC::TeensyESC() : pin(255), angle(NO_ANGLE) {}
...
void TeensyESC::writeMicroseconds(uint32_t usec)
{
    ...
    uint32_t duty = (int)(usec / 2500.0f * 65535.0f);
