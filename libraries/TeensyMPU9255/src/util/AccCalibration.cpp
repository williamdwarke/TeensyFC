...
uint8_t sample_size = 32;              // Number of measurements averaged to produce 1 sample
...
void take_sample(long *sample_out) {
    ...
    while (success == 0) {
        ...
        for (i = 0; i < (1 << first_pass_size); ++i) {
            //Take a reading
            readSPIRegs(accSS, MPU9255_ACCEL_XOUT_H, 6, &rawDataBuf[0]);
        ...
        if (i == sample_size) {
            ...
            Serial.print("# ");
            Serial.print(sample_out[0]);
            Serial.print(" ");
            Serial.print(sample_out[1]);
            Serial.print(" ");
            Serial.print(sample_out[2]);
            Serial.print(";\n");
        }
    }
}
